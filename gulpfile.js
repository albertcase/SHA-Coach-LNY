...
gulp.task('minifycss', function() {
    return gulp.src(['dist/src/css/reset.css', 'dist/src/css/swiper.min.css', 'dist/src/css/animate.min.css', 'dist/src/css/style.css'])                  //压缩的文件
    	 .pipe(concat('all.css'))                  //合并所有js到main.js
    	 .pipe(rename({suffix: '.min'}))           //rename压缩后的文件名
         .pipe(minifycss())                       //执行压缩
         .pipe(gulp.dest('dist/asset/css'));        //输出文件夹
});
...
gulp.task('minifyjs', ["publicjs"], function() {
    return gulp.src(['dist/src/js/jquery.js', 'dist/src/js/PxLoader.js', 'dist/src/js/tweenMax.min.js'])
        ...
        .pipe(rename({suffix: '.min'}))           //rename压缩后的文件名
        .pipe(uglify())                           //压缩
        .pipe(gulp.dest('dist/asset/js'));          //输出
});

gulp.task('publicjs', function() {
    return gulp.src(['dist/src/js/public.js', 'dist/src/js/element.js'])
...
gulp.task('minifyimg', function(){
    var miniImgSrc;
    if(!argv._imgml){
        miniImgSrc = ['dist/src/img/*.{png,jpg,gif,ico}']
    }else{
        miniImgSrc = ['dist/src/img/'+argv._imgml+'/*.{png,jpg,gif,ico}']
    }
...
gulp.task('tinypng', function () {
    gulp.src(['dist/src/img/*.{png,jpg,gif,ico}'])
...
gulp.task('clean', function(cb) {
    del(['dist/asset/css', 'dist/asset/js', 'dist/asset/img'], cb)
});
